...
#[cfg(test)]
use cli::{BundleCommand, GitCacheCommand, GitCommand, ManifestCommand, StoreCommand};
use error::CliError;
use format::{
...
fn run(cli: Cli) -> Result<String, CliError> {
    ...
    match cli.command {
        ...
        Some(Command::Store { command }) => commands::store::run_store_command(command, &paths),
        Some(Command::Daemon { command }) => {
            commands::daemon::run_daemon_command(command, &paths, require_daemon)
        }
        Some(Command::Web { command }) => commands::web::run_web_command(command, &paths),
        None => Ok(help_text()),
    }
}
