...
use kairo_statement::json::{ObjectRevisionBodyJson, ObjectRevisionStatementJson, SignatureJson};
...
fn inspect_output_includes_manifest_details() {
    ...
    let output =
        manifest.map(|manifest| crate::commands::manifest::format_manifest_inspection(&manifest));
...
fn formats_actor_genesis_verified_revision_output() {
    ...
    let output = actor_genesis.ok().and_then(|actor_genesis| {
        let statement = signed_revision_statement_for_actor(actor_genesis.actor_id().to_string())?;
...
fn revision_dto_for_actor(actor_id: String, manifest_hash: String) -> ObjectRevisionStatementJson {
...
#[test]
fn verify_object_with_real_git_repo_can_reach_valid() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(output.contains(&format!(
        "manifest_source = git:sha256:{commit_oid}/kairo.toml"
    )));
...
#[test]
fn verify_object_with_repo_missing_commit_is_invalid() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let revision_output = run(Cli {
        ...
        command: Some(Command::Revision {
            command: RevisionCommand::Create {
                actor: actor_id.clone(),
                object: object_id.clone(),
                revision: "git:sha256:0123456789abcdef0123456789abcdef01234567".to_owned(),
                manifest: manifest_path.clone(),
                parents: vec![],
                no_attests_reachable_history: false,
            },
...
#[test]
fn trust_block_then_withdraw_chains_correctly() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
...
#[test]
fn verify_object_auto_picks_sole_local_actor_for_trust() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn verify_object_with_explicit_as_grants_trusted() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn verify_object_with_no_as_skips_trust_evaluation() -> Result<(), Box<dyn std::error::Error>> {
    let (store_dir, _manifest_dir, _actor_id, object_id, _revision_statement, manifest_path) =
        fixture_with_branch()?;
...
#[test]
fn verify_object_ambiguous_local_actor_errors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(result, Err(CliError::AmbiguousLocalActor { .. })));
    Ok(())
}
...
fn create_local_actor(store_dir: &std::path::Path) -> Result<String, Box<dyn std::error::Error>> {
    parse_field(
        &run(Cli {
...
#[test]
fn capability_grant_then_list_by_grantor() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
...
#[test]
fn capability_grant_supersedes_prior_chain_leaf() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let grantor = create_local_actor(store_dir.path())?;
    let grantee = create_local_actor(store_dir.path())?;
    let object = create_local_object(store_dir.path(), &grantor)?;
    ...
    let second = run(Cli {
        ...
        command: Some(Command::Capability {
            command: CapabilityCommand::Grant {
                ...
                grantee: grantee.clone(),
                object: object.clone(),
                kinds: vec!["ObjectVersionTag".to_owned(), "ObjectBranch".to_owned()],
                delegable: true,
                expires_at: None,
                max_delegation_depth: None,
                key_pinned: None,
...
#[test]
fn capability_list_requires_exactly_one_filter() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let neither = run(Cli {
        store: Some(store_dir.path().to_path_buf()),
        keys: None,
...
#[test]
fn capability_grant_rejects_empty_kinds() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let grantor = create_local_actor(store_dir.path())?;
    let grantee = create_local_actor(store_dir.path())?;
    let object = create_local_object(store_dir.path(), &grantor)?;
...
#[test]
fn capability_list_by_object_includes_grantor_grantee() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let grantor = create_local_actor(store_dir.path())?;
    let grantee = create_local_actor(store_dir.path())?;
    let object = create_local_object(store_dir.path(), &grantor)?;
...
#[test]
fn revoke_key_refuses_to_brick_actor_without_flag() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
...
#[test]
fn signing_command_after_rotation_uses_new_active_key() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn actor_create_with_operator_presented_attestation_key() -> Result<(), Box<dyn std::error::Error>>
{
    let store_dir = tempfile::TempDir::new()?;
    ...
    let attestation_pub = SigningKey::from_bytes(&attestation_seed)
        .verifying_key()
        .to_bytes();
    let attestation_hex: String = attestation_pub.iter().map(|b| format!("{b:02x}")).collect();
...
#[test]
fn actor_create_generate_attestation_key_prints_seed_once() -> Result<(), Box<dyn std::error::Error>>
{
    let store_dir = tempfile::TempDir::new()?;
    let output = run(Cli {
        store: Some(store_dir.path().to_path_buf()),
...
fn actor_recover_key_sign_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let logged_attestation_key_id = parse_field(&recover_output, "attestation_key_id = ")?;
    ...
    let rotate_output = run(Cli {
        ...
        offline: false,
        command: Some(Command::Actor {
            command: ActorCommand::RotateKey { actor: actor_id },
        }),
    })?;
    assert!(rotate_output.contains("rotated key"));
    Ok(())
...
#[test]
fn actor_recover_key_prepare_import_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let create_output = run(Cli {
        store: Some(store_dir.path().to_path_buf()),
        keys: None,
...
#[test]
fn actor_recover_key_cosign_two_of_two_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
...
#[test]
fn actor_add_attestation_key_sign_generate_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
    let create_output = run(Cli {
        store: Some(store_dir.path().to_path_buf()),
        keys: None,
    ...
    let actor_id = parse_field(&create_output, "actor = ")?;
    let seed_b64 = parse_field(&create_output, "    seed = ")?;
    let initial_attestation_key_id = parse_field(&create_output, "    attestation_key_id = ")?;
    ...
    assert_eq!(
        history["attestation_adds"].as_array().map(Vec::len),
        Some(1)
    );
    Ok(())
}
...
#[test]
fn actor_revoke_attestation_key_sign_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    let store_dir = tempfile::TempDir::new()?;
...
fn kairo_git_cache_status_after_fetch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        output.contains("pool: initialized"),
        "expected initialized pool, got:\n{output}"
    );
...
fn kairo_bundle_export_include_git_writes_pack_and_flips_flag(
    ...
    let pack_path = bundle_path.join("git").join(format!("{object_id}.pack"));
    assert!(
        pack_path.is_file(),
        "expected pack at {}",
        pack_path.display()
    ...
    let manifest_str = std::fs::read_to_string(bundle_path.join("manifest.json"))?;
    assert!(
...
fn kairo_bundle_roundtrip_with_git_packs_verifies_without_cwd_repo(
    ...
    assert!(
        status_output.contains(&format!("refs/kairo/imported/{commit_oid} = {commit_oid}")),
...
#[test]
fn kairo_bundle_import_without_git_data_skips_cache_open() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let import_output = run(Cli {
        ...
        offline: false,
        command: Some(Command::Bundle {
            command: BundleCommand::Import { input: bundle_path },
        }),
    })?;
